...
class ExportActionReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        android.util.Log.d(
            "ExportDebug",
            "📩 ExportActionReceiver received action: ${intent?.action}"
        )
        ...
        when (intent?.action) {
            "PAUSE" -> {
                ...
                if (workerId != null) {
                    ...
                    android.util.Log.e(
                        "ExportDebug",
                        "❌ PAUSE failed: workerId is null! Cannot cancel worker."
                    )
                    // Try to cancel by tag as fallback
                    android.util.Log.d(
                        "ExportDebug",
                        "⏸️ Attempting to cancel by tag 'video_export'"
                    )
            ...
            "RESUME" -> {
                ...
                android.util.Log.d(
                    "ExportDebug",
                    "▶️ Resume paths - projectData: $projectDataPath, settings: $exportSettingsPath"
                )

                if (projectDataPath != null && exportSettingsPath != null) {
                    // Verify files exist before attempting resume
                    val projectDataFile = File(projectDataPath)
                    val settingsFile = File(exportSettingsPath)

                    if (!projectDataFile.exists()) {
                        android.util.Log.e(
                            "ExportDebug",
                            "❌ Project data file not found: $projectDataPath"
                        )
                        return
                    }
                    if (!settingsFile.exists()) {
                        android.util.Log.e(
                            "ExportDebug",
                            "❌ Settings file not found: $exportSettingsPath"
                        )
                        return
                    }
                    ...
                    android.util.Log.d(
                        "ExportDebug",
                        "📤 Enqueuing resume worker with ID: ${request.id}"
                    )
            ...
            "CANCEL_PAUSED" -> {
                ...
                if (exportSettingsPath != null) {
                    try {
                        ...
                        android.util.Log.e(
                            "ExportDebug",
                            "Failed to read export settings for cleanup",
                            e
                        )
                    }
                }
